...
class Solution {
    ...
    private void dfs(int n, int m, int x, int y, int r, int c, int k, int size, String path) {
        if (Answer.length() != 0 || (size + getDistance(x,y,r,c) > k)) {
            return;
        }
        if (size == k && x == r && y == c) {
            Answer = path;
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (inRange(nx, ny, n, m)) {
                dfs(n,m,nx,ny,r,c,k,size+1,path+Path[i]);
            }
        }
    }
